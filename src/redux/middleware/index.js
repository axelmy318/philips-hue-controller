    ...
    return next => action => {
      ...
      if(action.payload){
        ...
        if(action.action){
          ...
          payload = (action.payload !== undefined && action.payload !== null) ? action.payload : {}
        }
        else{
          payload = (action.payload !== undefined && action.payload !== null) ? action.payload : {}
        }
      ...
      const handleReject = reason => {
        ...
        rejectedAction.data = data ? data : null
      ...
      const handleFulfill = (value = null) => {
        const action = getAction(value, false);
      ...
      let handledPromise = promise.then(handleFulfill, handleReject)
      return {handledPromise, data}
    };
  };
}
